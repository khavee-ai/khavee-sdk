...
- Live human verification still pending for 260707-oyu (Avatar two-column layout, mock chat containment, transparent-toggle fix) — automated checks (php -l, harnesses, tsc, wp-bundle build) all pass, but the fix needs to be exercised live in wp-env (http://localhost:8888/wp-admin/admin.php?page=khaveeai-settings) to confirm the transparent-toggle bug is actually resolved for the end user
...
| 260707-oyu | Settings page follow-up fixes: Avatar section preview moved into a two-column layout (mirrors Floating Widget), mock chat bubbles rewired into a real chat-box wrapper (header + bounded transcript) matching ChatBox.tsx, and a genuine source-verified fix for the transparent-toggle bug — disproved the prior `key`-prop-remount hypothesis (commit 5a39d51) via direct trace of the installed react-three-fiber/three.js source, found the remount was a functionally-inert no-op causing WebGL-context churn, and fixed by removing the key + making the Canvas `gl` prop constant so the container CSS is the sole background-switching mechanism. All automated checks pass; live human verification pending | 2026-07-07 | 3f4fa74 (merge 5467395) | [260707-oyu-settings-page-follow-up-fixes](./quick/260707-oyu-settings-page-follow-up-fixes/) |
