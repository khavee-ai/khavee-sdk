...
## Open Questions (RESOLVED — Q1 in plan 07-01 Task 2; Q2 approach in plan 07-03 Task 2, empirical A2 confirmation in 07-03 Task 3 checkpoint)

1. **RESOLVED (07-01 Task 2):** Does storing the avatar as an attachment ID require modifying `WpOptionsConfigSource.php` (Phase 6 code), or should this phase store a pre-resolved URL string to avoid touching existing code at all?
...
2. **RESOLVED (07-03 Task 2 approach + 07-03 Task 3 empirical checkpoint):** Exact hook/timing for scoping `upload_mimes`/`wp_check_filetype_and_ext` around the `wp.media`-driven upload, given that the actual file upload happens via WordPress core's own AJAX handler, not this plugin's form-submit handler.
