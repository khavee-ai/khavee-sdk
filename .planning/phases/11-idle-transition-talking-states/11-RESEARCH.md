...
> RESOLVED: Both questions are resolved by Phase 11 planning decisions. Q1 (VRM `starting`/`stopped` placeholder) -> Plan 05 Task 1: `starting` uses the duration-floor mechanism only (no VRM greeting clip exists), and `stopped` is delivered as a procedural SETTLE cue (Plan 05 Task 1 decision, Plan 05 Task 2 implementation) on BOTH formats since no goodbye clip matches on either. The distinct dedicated-clip portion for both formats stays deferred to issue #17 (ASSET-01). Q2 (fixed composition order + bounded magnitude) -> Plan 05 Task 2: fixed order breathing -> sway -> post-composition spine clamp via `MAX_COMBINED_SPINE_DELTA_RAD` (post-composition clamp chosen, per the recommendation below).
